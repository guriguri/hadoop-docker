...
from templates import yarnfile, runner, stopper
...
def main(args):
	...
	hdfspath = os.getenv("LLAP_PKG_HDFS_PATH")
	if not hdfspath:
		hdfspath = ".yarn/package/LLAP/"
	...
	with open(join(output, "stop.sh"), "w") as f:
		f.write(stopper % vars)
	os.chmod(join(output, "stop.sh"), 0700)
